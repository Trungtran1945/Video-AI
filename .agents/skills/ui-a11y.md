...
- For general design system compliance review ΓåÆ use `/ss-review`
- For Nielsen UX heuristics ΓåÆ use `/ss-audit`
- For non-StyleSeed code (no `data-slot`, no semantic tokens) ΓÇö assumes StyleSeed conventions
- For runtime testing ΓÇö this is a static code audit, not a screen-reader simulation
...
- **Touch targets**: Minimum 44x44px (`min-h-11 min-w-11`)
  - Common violation: `h-9` (36px) buttons ΓÇö should be `h-11`
...
|-------|-----------------|------|
| `--foreground` | 7:1+ | Body text ΓÇö verify with your skin |
| `--muted-foreground` | 4.5:1+ | Secondary text ΓÇö verify with your skin |
| `--brand` | 4.5:1+ | Accent ΓÇö verify with your skin's brand color |
| `--destructive` | 4.5:1+ | Error ΓÇö verify with your skin |
| `--success` | 3:1+ | Large text/icons only ΓÇö verify with your skin |
| `--warning` | 4.5:1+ | Warning text ΓÇö some skins need a darker variant |
