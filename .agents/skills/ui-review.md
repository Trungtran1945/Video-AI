...
- For accessibility-only issues ΓåÆ use `/ss-a11y`
- For Nielsen UX heuristics ΓåÆ use `/ss-audit`
- For a quick automated check ΓåÆ use `/ss-lint`
...
### 8. Coherence (VISUAL-CRAFT.md ┬ºC0 ΓÇö the "one choice per axis" laws)
> The biggest reason a UI reads as "AI-generated" isn't ugly parts ΓÇö it's *mixed*
> parts. Check that each axis below uses ONE value system-wide; flag a mix as a real
> issue, not a nitpick.
- [ ] **One radius personality** ΓÇö sharp (0-4px) OR soft (8-12px) OR pill, applied to every card/button/input/modal. No mixing (e.g. a `rounded-none` panel with `rounded-full` buttons).
- [ ] **One accent color** for interactive emphasis (+ semantic red/green/amber only) ΓÇö not two+ competing accents.
- [ ] **No emoji as UI icons** (≡ƒÜù≡ƒº║Γ¡É as list/nav/status/category markers) ΓÇö they inject many uncontrolled hues; use one line-icon set in `currentColor`.
- [ ] **Status color = severity, not decoration** ΓÇö a normal/OK/"δ│┤φå╡" state is neutral grey (not colored); color marks only the minority of rows that need attention; same value ΓåÆ same color.
- [ ] **No decorative hues** ΓÇö favorite stars, category dots, avatars use the accent or grey, not a new color each.
- [ ] **One shadow language** ΓÇö same light direction, same scale/tint; not some black + some tinted, some up-lit + some down-lit.
- [ ] **One icon family / fill mode / stroke weight** across the file.
- [ ] **Nested-radius law** ΓÇö an element inside a rounded container uses `inner = outer ΓêÆ padding`, not the same radius (which bulges).
- [ ] **Consistent control heights** ΓÇö buttons, inputs, selects share a height set (e.g. 40px).
