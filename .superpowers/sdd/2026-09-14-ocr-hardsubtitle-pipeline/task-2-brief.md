# Task 2 Brief: Create dubOcr.js Pipeline Stage

## Task Description

Create a new pipeline stage at `backend/src/pipeline/stages/dubOcr.js` that:
1. Extracts frames from the source video at 1 FPS
2. Runs OCR on each frame using the TesseractOcr provider (improved in Task 1)
3. Aggregates OCR results across frames into timeline segments
4. Writes results to `transcript_segments` table (same as ASR/stt does)

## Context

This stage replaces `dub.stt` (ASR) when the user selects OCR mode. The rest of the pipeline (translate → TTS → render) reads from `transcript_segments.text`, so this stage must write to the same table with the same schema.

## Interfaces

### Consumes
- `ctx.project` — project row with `id`, `source_video_key`, `user_id`, `params` (JSON string)
- `ctx.results['dub.ingest']` — `{ durationSec, width, height, normalizedAudioKey }`
- `ctx.job` — job row (for logging)
- `ctx.setProgress` — progress callback
- `ctx.signal` — abort signal

### Produces
- Writes rows to `transcript_segments` table: `{ id, project_id, index_num, start_sec, end_sec, text, speaker, language }`
- Returns `{ segmentCount, language }`

## Implementation

Create `backend/src/pipeline/stages/dubOcr.js` with this exact content:
...
```bash
cd backend && node -e "import('./src/pipeline/stages/dubOcr.js').then(() => console.log('OK'))"
```
Expected: `OK`
