...
## Scan Time
2026-09-18T01:19:00Z

## Project Health
The Video_AI codebase has strong architectural modularity with stage-based runners, provider registries, and solid unit coverage across media functions. However, several concurrency, resilience, and security issues exist in the backend services. Specifically, `runPipeline` suffers from an asynchronous race condition permitting duplicate pipeline runs, key decryption in `crypto.decrypt` throws unhandled authentication tag exceptions crashing the API keys endpoint, YouTube upload id generation references an undeclared `crypto` module, password validation accepts single-character credentials, and provider queries expose unassociated system/test logs across all authenticated accounts.

## Findings

### BUG
- [backend/src/pipeline/runner.js:456-470](file:///D:/E/Video_AI/backend/src/pipeline/runner.js#L456-L470) â€” `activeRuns.add(projectId)` is called after asynchronous `await queryOne()`, creating an event-loop race window where concurrent requests launch duplicate pipeline runs on the same project.
- [backend/src/routes/v1/projects.js:311-326](file:///D:/E/Video_AI/backend/src/routes/v1/projects.js#L311-L326) â€” `POST /api/v1/projects/:id/regenerate` does not verify `isPipelineRunning()`, permitting concurrent pipeline triggers while a run is active.
- [backend/src/routes/v1/generation.js:70-92](file:///D:/E/Video_AI/backend/src/routes/v1/generation.js#L70-L92) â€” `POST /api/v1/projects/:id/jobs/:type/retry` does not check `isPipelineRunning()`, risking uncoordinated re-execution during active pipeline execution.
...
- [backend/src/routes/v1/outputs.js:54](file:///D:/E/Video_AI/backend/src/routes/v1/outputs.js#L54) â€” `crypto.randomUUID()` is called in `POST /api/v1/outputs/:id/youtube` without importing `crypto` from `node:crypto`.
...
- [backend/src/routes/v1/auth.js:24-27,133-134](file:///D:/E/Video_AI/backend/src/routes/v1/auth.js#L24-L27) â€” Registration and password reset only check for presence/truthiness of passwords, allowing single-character passwords without length validation.

### PERFORMANCE
- [backend/src/media/ffmpeg.js:66,73,88](file:///D:/E/Video_AI/backend/src/media/ffmpeg.js#L66) â€” `setTimeout(() => { try { child.kill('SIGKILL') } catch (_) {} }, 5000)` creates unreferenced timers that keep the Node.js event loop unnecessarily active for 5 seconds after process termination.
