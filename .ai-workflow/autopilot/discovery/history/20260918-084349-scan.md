...
## Scan Time
2026-09-18T01:44:00Z

## Project Health
The codebase exhibits clean architectural boundaries across its BullMQ Redis worker subsystem, SQLite/sql.js persistence layer, and dual-mode execution engines (`TRANSLATE_DUB` and `SUMMARY`). However, critical operational risks remain: OCR subtitle bounding boxes are discarded in `dubOcr` preventing subtitle masking in `dubRender`, SQLite data is mapped outside the mounted Docker volume causing data loss on container recreation, unchecked ASR segment arrays risk runtime `TypeError` crashes, and unnormalized mode keys cause pipeline resume lookup misses in the retry endpoint. Immediate remediation of these issues will stabilize end-to-end media generation and deployment durability.

## Findings

### BUG
- [`dubOcr.js:80-84`](file:///D:/E/Video_AI/backend/src/pipeline/stages/dubOcr.js#L80-L84) â€” OCR tracks maintain bounding box coordinates but drop them when inserting `transcript_segments`, leaving `ocr_regions` empty and causing [`dubRender.js:47-53`](file:///D:/E/Video_AI/backend/src/pipeline/stages/dubRender.js#L47-L53) to fall back to screen bottom (`\an2`) instead of masking original hardsubs.
...
### SECURITY
- [`quotaGuardService.js:109-114`](file:///D:/E/Video_AI/backend/src/services/quotaGuardService.js#L109-L114) â€” `selectBestApiKey` queries `provider_logs` for `status = 'rate_limited'` across all users globally without user or key filtering, causing one user's quota limit to throttle all keys for all users.
...
### TEST
- [`alignService.js:57`](file:///D:/E/Video_AI/backend/src/pipeline/alignService.js#L57) â€” Zero automated test coverage exists in `backend/tests/` for `SUMMARY` mode algorithms (`packSegment`, `buildTimelineRows`, `buildCuesFromWindows`), leaving packaging and timing logic vulnerable to regression (covered in pending `TASK-025`).
...
## Recommended Tasks
1. TASK-025 Add unit test coverage for SUMMARY mode packSegment and timeline assembly (PENDING in backlog)
2. TASK-026 Persist OCR bounding boxes into ocr_regions in dubOcr to restore subtitle mask positioning in dubRender
3. TASK-027 Fix SQLite database persistence path in docker-compose.yml to prevent data loss on container recreation
4. TASK-028 Guard against null or missing segments in summaryTranscribe stage
5. TASK-029 Normalize project mode before looking up STAGES in generation retry endpoint
