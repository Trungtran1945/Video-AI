...
## Scan Time
2026-09-18T01:55:00Z

## Project Health
The project maintains well-structured execution pipelines across dual modes (`TRANSLATE_DUB` and `SUMMARY`), supported by BullMQ queues and SQLite persistence. However, critical operational and consistency bugs remain: unnormalized mode strings in `runner.js` cause projects with mode variants such as `translate-dub` to skip all pipeline stages and immediately mark the project as 100% completed without generating media outputs; un-scoped rate-limit queries in `quotaGuardService` trigger global cross-tenant throttling; synchronous directory scans in `dubRender` introduce redundant disk I/O during segment resolution; and quadratic linear scans in `dubTtsAlign` degrade performance on large transcript sets. Addressing these defects will significantly improve runtime correctness and pipeline reliability.
...
### PERFORMANCE
- [`dubRender.js:87-92`](file:///D:/E/Video_AI/backend/src/pipeline/stages/dubRender.js#L87-L92) â€” `scanForSegment(segmentId)` invokes `fs.readdirSync(segDir)` on every call during audio entry matching, causing repeated synchronous filesystem directory reads instead of inspecting a cached list or existing map.
...
### REFACTOR
- [`dubRender.js:80-85,93-97`](file:///D:/E/Video_AI/backend/src/pipeline/stages/dubRender.js#L80-L85) â€” `filesById` and `byAudioOrSegment` maps are populated from directory entries but never queried by the resolution logic.

### FEATURE OPPORTUNITY
- [`Queue.jsx:16-27`](file:///D:/E/Video_AI/frontend/src/pages/Queue.jsx#L16-L27) â€” The Queue page fetches job statuses only once on mount; adding auto-refresh polling and a manual reload action would provide live queue tracking.

## Recommended Tasks
1. TASK-030 Normalize project mode and guard against empty stage groups in pipeline runner and regenerate endpoint
2. TASK-031 Isolate rate limit query by user in quotaGuardService to prevent cross-user key throttling
3. TASK-032 Cache directory entries in dubRender audio resolver to eliminate repeated synchronous filesystem scans
4. TASK-033 Optimize quadratic segment lookup in dubTtsAlign placement mapping
5. TASK-034 Add auto-refresh polling and manual reload to Queue page
